...
def detect_and_display(frame):
    ...
    return person_detected, person_position
...
def image_callback(image_msg):
    ...
    person_detected, person_position = detect_and_display(cv_image)
    if person_detected:
        rospy.loginfo("Persona detectada")
        # Publicar la posición de la persona
        pos_msg = Twist()
        pos_msg.linear.x = person_position[0]
        pos_msg.linear.y = person_position[1]
...
if __name__ == '__main__':
    ...
    pos_pub = rospy.Publisher('/person_position', Twist, queue_size=10)
    bridge = CvBridge()
